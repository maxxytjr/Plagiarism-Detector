...
def create_datatype(df, train_value, test_value, datatype_var, compare_dfcolumn, operator_of_compare, value_of_compare,
    ...
    # df_subset = df[operator.gt(df['Category'], 0)]...if value in category column is greater than (gt) 0 (value_of_compare)
    # Hence in here, we are creating a subset of the df for observations that are plagiarized (1, 2, or 3)
